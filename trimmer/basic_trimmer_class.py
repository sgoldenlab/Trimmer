...
class trim_list:
    ...
    def add_trim_points(self, trial_idx, trim_start, trim_end, trim_label):
        if any(
            [np.isnan(ix) for ix in self.trim_pts_array[:, 0]]
        ):  # if any uninitialized rows
            nan_idx = np.where([np.isnan(ix) for ix in self.trim_pts_array[:, 0]])[0][
                0
            ]  # get first index where above is true
            self.trim_pts_array[nan_idx] = np.array(
                [[trial_idx, trim_start, trim_end]]
            )  # replace nan values if any
            self.trim_pts_labels[nan_idx] = np.array([[trial_idx, trim_label]])
        else:
            self.trim_pts_array = np.append(
                self.trim_pts_array,
                np.array([[trial_idx, trim_start, trim_end]]),
                axis=0,
            )
            self.trim_pts_labels = np.append(
                self.trim_pts_labels, np.array([[trial_idx, trim_label]]), axis=0
            )
        ...
        for i, t in enumerate(self.trim_pts_array[self.trim_pts_array[:, 0].argsort()]):
            try:
                self.printout.append(
                    f'trial {int(t[0]) + 1}, "{self.trim_pts_labels[i, 1]}": {self.frame_to_time(t[1], fps)}-{self.frame_to_time(t[2], fps)}'
                )
    ...
    def remove_selected_tp(self, listbox_element: sg.Listbox):
        ...
        if to_remove is not []:
            for item in to_remove:
                ...
                self.trim_pts_labels = np.delete(
                    self.trim_pts_labels, remove_idx, axis=0
                )
...
class vid_info:
    def __init__(
        self, fps, tot_frames, height, width, trial_num, cur_frame, ret, frame
    ):
        self.fps = fps
...
def buildWindow(debug=False):  # TODO: disable text boxes, buttons etc as relevant
    ...
    vid_column = [
        ...
        [
            ...
            place_noex(
                sg.Button("-5", font="Arial 10", k="bPrevF5", auto_size_button=True)
            ),
            place_noex(
                sg.Button("-1", font="Arial 10", k="bPrevF", auto_size_button=True)
            ),
            place_noex(
                sg.Button("+1", font="Arial 10", k="bNextF", auto_size_button=True)
            ),
            place_noex(
                sg.Button("+5", font="Arial 10", k="bNextF5", auto_size_button=True)
            ),
            place_noex(
                sg.Button("+30", font="Arial 10", k="bNextF30", auto_size_button=True)
            ),
            ...
            # sg.Push(),
            place_noex(
                sg.Button("Reset", font="Arial 10", k="bReset", auto_size_button=True)
            ),
            place_noex(
                sg.Button(
                    "Check F",
            ...
            if debug
            else place_noex(sg.Txt("", font="Arial 10", size=(5, 1))),
            place_noex(
                sg.Button("Next Video", font="Arial 10", k="bNV", auto_size_button=True)
            ),
        ],
    ]
    right_test = [
        ...
        [
            ...
            sg.Column(
                [
                    [sg.Input("", key="-input-resize-", font="Arial 10", size=(5, 1))],
    ...
    for i in range(1, 13):
        trim_col += (
            [
                sg.T(f"Trial {i}", font="Arial 8", size=(6, 1), justification="left"),
            ...
            sg.Column(
                [
                    ...
                    [
                        ...
                        sg.Push(),
                        place_noex(
                            sg.B("Exit", font="Arial 10", auto_size_button=True)
                        ),
                    ],
                ],
                expand_y=True,
    ...
    video_layout = [
        [
            sg.Column(right_test, k="vid_column", expand_x=False),
    ...
    # final layout
    layout = [
        [sg.Column(top_region, k="top_region", expand_x=True)],
...
def scan_video_folder(
    source_dir: str, out_dir: str, usable_extensions=[".mp4"], debug=False
):
    ...
    if metadata is not None:
        ...
        print("No metadata found for videos in chosen folder.\n")
    video_list = [
        video
        for video in os.listdir(source_dir)
        if video.endswith(usable_extensions) and video not in processed_videos
    ]
    return video_list, processed_videos


def reset_info(
    window: sg.Window, filename: str, filepath: str, out_folder="./clips"
) -> sg.Window:
...
def main(debug=False):
    ...
    while True:
        ...
        if (
            event == "-SLIDER-" and timeout == None
        ):  # is this for slider event at end of video?
            timeout = 1000 // fps
        ...
        if event == "-FOLDERI-" and timeout == None:
            ...
            os.chdir(
                folder_in
            )  # probably not needed, since we want to work with absolute paths or relative to source
            window[
                "bNV"
            ].click()  # autoclicks next video; start with first video loaded in
        ...
        if event == "bNV":
            ...
            if values["-FOLDERI-"] == "":
                print(
                    'No source folder selected, select one under "Videos Folder" and try again.'
                )
                continue
        ...
        window["-preview-"].update(
            value="_".join(preview_str)
        )  # combine string for output based on info values

        # clear inputs when 'focused' on
        if event in input_focusin:
            input_elem = event.split("_")[0]
            input_defaults[inputs.index(input_elem)] = values[
                input_elem
            ]  # temporary workaround to saving prev text
            window[input_elem].update(value="")
            window[input_elem].update(text_color="black")

        elif (
            event in input_focusout
        ):  # not bad though, currently leaves last input if unchanged
            input_elem = event.split("_")[0]
            if (
                values[input_elem] == ""
            ):  # if no text typed in, go back to previous value
                window[input_elem].update(
                    value=input_defaults[inputs.index(input_elem)]
                )
            else:
        ...
        if ("bNextF" in event) or ("bPrevF" in event) or ("Reset" in event):
            ...
            if vid.cur_frame >= vid.tot_frames:
                print(
                    'This is the end of the video. Please click "Next Video" to advance'
                )
            slider_elem.update(vid.cur_frame)
            window["sleft"].update(
                f"{strftime('%M:%S', gmtime(vid.cur_frame / vid.fps))} .{int((vid.cur_frame % vid.fps) * 3.3):02d}"
            ...
            try:
                ...
                if vid.cur_frame > vid.tot_frames:
                    print(
                        "There are only",
                        int(vid.tot_frames),
                        "frames in the video. Please select a frame within the video",
                    )
                    continue
                slider_elem.update(vid.cur_frame)
                window["sleft"].update(
        ...
        # "Next Video" button => update info on slide with next video
            ...
            for i in range(vid.trial_num):
                ...
                if (start_frame > vid.tot_frames) or (end_frame > vid.tot_frames):
                    ...
                    window[trial_list[i] + "_start"].update(
                        t.frame_to_time(start_frame, vid.fps), text_color="black"
                    )
                    window[trial_list[i] + "_end"].update(
                        t.frame_to_time(end_frame, vid.fps), text_color="black"
                    )
        ...
        ]:  # if goto button pressed, go to start frame of trim range
            ...
            slider_elem.update(vid.cur_frame)
            window["sleft"].update(
                f"{strftime('%M:%S', gmtime(vid.cur_frame / vid.fps))}.{int((vid.cur_frame % vid.fps) * 3.3)}"
        ...
        elif event in [f"{i}_start" for i in trial_list] or event in [
            f"{i}_end" for i in trial_list
        ]:
            trial_idx = trial_list.index(
                event.split("_")[0]
            )  # get index of trial for indexing arrays
            if debug:
                print(
                    f"updating {event} to display frame {vid.cur_frame} as {t.frame_to_time(vid.cur_frame, vid.fps)}"
                )
            try:
                if "start" in event:
        ...
        # add trim points button pressed - per trial
            ...
            num_clips = len(
                t.trim_pts_array[np.where(t.trim_pts_array[:, 0] == trial_idx)]
            )
            window[trial_list[trial_idx] + "_label"].update(f"clip_{num_clips}")

            t.get_printed_trimpoints_listbox(
                vid.fps
            )  # get trimpoints into list of strings for listbox
            window["-tpts_out-"].update(
                t.printout
            )  # print out current trimpoints added to 2nd notes textbox

            # set start tp box to end point of added trimpoint, and end tp to end of trial again - simplifies workflow
            window[f"{trial_list[trial_idx]}_start"].update(
                t.frame_to_time(t.trim_ends[trial_idx], vid.fps)
            )
            ...
            if t.trim_starts[trial_idx] > original_t_ends[trial_idx]:
                if debug:
                    print(
                        "Trim points are both past trial end, or end time is earlier than start time"
                    )
                window[f"{trial_list[trial_idx]}_start"].update(text_color="red")
                window[f"{trial_list[trial_idx]}_end"].update(
                    t.frame_to_time(t.trim_starts[trial_idx], vid.fps), text_color="red"
                )
                t.trim_ends[trial_idx] = t.trim_starts[trial_idx]
            else:  # all good
                window[f"{trial_list[trial_idx]}_end"].update(
                    t.frame_to_time(original_t_ends[trial_idx], vid.fps)
                )
                t.trim_ends[trial_idx] = original_t_ends[trial_idx]
            print(
                f"Set current start point for trial {trial_idx + 1} to end of clip added, and end point to original trial end time"
            )
            ...
            if t.trim_starts == []:
                ...
                "This will add trim points for ALL trials based on their current times and label names \nWould you like to continue?",
                "yesno",
            ):
                for i in range(len(t.trim_starts)):
                    ...
                    if t.trim_starts[trial_idx] > original_t_ends[trial_idx]:
                        ...
                        window[f"{trial_list[trial_idx]}_end"].update(
                            t.frame_to_time(t.trim_starts[trial_idx], vid.fps),
                            text_color="red",
                        )
                        t.trim_ends[trial_idx] = t.trim_starts[trial_idx]
        ...
        # remove selected trim points
            ...
            if to_remove == []:
                popup(
                    "You have not selected any trim points to remove. Please select trim points to remove and try again."
                )
            else:
                if popup(
                    "This will remove the trim point(s): \n"
                    + "\n".join(to_remove)
                    + "\n\nWould you like to continue?",
                    "yesno",
                ):
                    if t.printout != []:
                        ...
                        print(
                            "Tried to remove trimpoints but no trimpoints have been added."
                        )
        # clear all trim point boxes
        elif event == "-clr-":
            if popup(
                "This will clear all trim point boxes AND added trim points. \nWould you like to continue?",
                "yesno",
            ):
        ...
        # clear added trim points
            ...
            if popup(
                "This will clear all the added trim points and reset the trim point boxes. \nWould you like to continue?",
                "yesno",
            ):
                t.trim_pts_array = np.ndarray((vid.trial_num, 3))
                t.trim_pts_array[:] = np.nan
        ...
        # save click
            ...
            start_col = pd.Series(
                t.frame_to_time(t.trim_pts_array[:, 1].tolist(), vid.fps), name="Start"
            )
            end_col = pd.Series(
                t.frame_to_time(t.trim_pts_array[:, 2].tolist(), vid.fps), name="End"
            )
            ...
            out_csv = pd.concat(
                [trial_col, label_col, start_col_tabs, end_col_tabs], axis=1
            )
            out_csv.to_csv(
                f"{out_base}.csv",
                index=True,
                index_label="npy_array_index",
                date_format="%M:%S",
            )
            ...
            video_info = {
                ...
                "FPS": vid.fps,
                "Duration (h:min:sec)": strftime(
                    "%H:%M:%S", gmtime(vid.tot_frames / vid.fps)
                ),
            }
            expt_info = {
                ...
                "session": values["-sess-"],
                "date": datetime.strptime(values["-date-"], "%y%m%d").strftime(
                    "%Y.%m.%d"
                ),
            }
            clips_details = {}
